...
Set.prototype.add = function(element) {
	if(!this.contains(element)) {
		...
		return this._copy(this._elements.concat([element]));
	} else {
		return this;
	}
};
...
Set.prototype.union = function(set) {
	return this._copy(this._elements.concat(set._elements));
};

Set.prototype.map = function(fn) {
	var elements = [];
	for(var i = 0; i < this._elements.length; i++) {
		elements.push(fn(this._elements[i]));
	}
